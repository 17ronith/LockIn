...
class PlaylistRanker:
    ...
    def rank_playlist(self, playlist_url: str, user_intent: str) -> List[Dict]:
        ...
        if thumbnail_images:
            visual_embeddings_batch = self.visual_model.encode(
                thumbnail_images, 
            ...
            if len(visual_embeddings_batch.shape) != 2:
                logger.error(f"Unexpected visual embeddings shape: {visual_embeddings_batch.shape}")
                raise ValueError("Visual embeddings have inconsistent shapes")
            
            # Ensure all embeddings are the expected dimension
